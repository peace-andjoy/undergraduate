...
data = pickle.load(open('en.pkl','rb'))
data = data[data['ab_words'].notnull()]
data = data[(data['Publication Year'] < 2020) & (data['Publication Year'] >= 2004)]
...
time_slice = data['Publication Year'].value_counts().sort_index().values.tolist()
num_topics = 40
...
df.to_csv('output_new/journal%d.csv' %num_topics, index = False)

with open ("output_new/journal%d.txt" %num_topics, 'wb') as f: #打开文件
